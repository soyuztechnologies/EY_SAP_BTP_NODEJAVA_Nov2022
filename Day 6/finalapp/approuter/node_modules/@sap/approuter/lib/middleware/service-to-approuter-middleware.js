...
module.exports = async function (req, res, next) {
  ...
  if (req.headers['x-approuter-authorization'].startsWith('Basic')){
    let ownCredentials = vcapUtils.getServiceCredentials({label: 'identity'});
    if (ownCredentials) {
      ownCredentials.label = 'ias';
    }
    if (!ownCredentials) {
    ...
    try {
      ...
      if (req.requiredRoleCollection && (!tokenResponse.tokenDecoded.roleCollections ||
        ...
        return writeAuditLog(req, creds.user , `Token validation failed for request url ${req.url}, missing required role collection ${req.requiredRoleCollection}`, 403, creds.user, ownCredentials, next);
      }
      req.headers['x-approuter-authorization'] = `Bearer ${tokenResponse.accessToken}`;
      return handleSession(req,tokenResponse.tokenDecoded, tokenResponse.accessToken,tracer,next);
    } catch (err){
      return writeAuditLog(req, creds.user , `${err.message} - Token validation failed for request url ${req.url}`, 401, creds.user, ownCredentials, next);
    }
  } else {
    let tokenDecoded;
    try {
    ...
    xssec.createSecurityContext(tokenValue, config, service.type, async function (err) {
      if (err) {
        ...
        return handleSession(req, tokenDecoded, tokenValue, tracer, next);
      }
    });
  }
};
...
async function handleSession(req, tokenDecoded, tokenValue,tracer, next){
