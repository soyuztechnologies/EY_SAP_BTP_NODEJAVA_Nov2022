...
module.exports = function cacheServiceDestination(req, res, next) {
  ...
  cacheBSDestinations(req)
    ...
    .then(() => setDynamicLogLevel(req, res))
    .then(() => next())
    .catch((err) => {
      next(err);
    });
...
function downloadApplication(req, res) {
  return new Promise((resolve,reject) => {
    ...
    if (req.headers['x-approuter-authorization'] && parsedUrl.path.startsWith('/applications/content')) {
      checkApprouterAuthorization(req).then(()=>{
...
function getApplicationLogs(req, res) {
  return new Promise((resolve,reject) => {
    ...
    if (req.headers['x-approuter-authorization'] && parsedUrl.path.startsWith('/applicationLogs')){
      checkApprouterAuthorization(req)
        .then(() => {
...
function setDynamicLogLevel(req, res) {
  return new Promise((resolve, reject) => {
    const parsedUrl = urijs.parse(req.url);
    if (req.headers['x-approuter-authorization'] && (parsedUrl.path === '/dynamicLogLevel/info' || parsedUrl.path === '/dynamicLogLevel/debug'
      || parsedUrl.path === '/dynamicLogLevel')) {
      logRequestInfo(req,`Setting dynamic log level for url ${parsedUrl.path}`);
      req.requiredRoleCollection = process.env.LOG_SESSION_ROLE_COLLECTION;
      checkApprouterAuthorization(req)
        .then(() => {
          if (req.method === 'POST') {
            let logLevel;
            if (parsedUrl.path.endsWith('info')) {
              logLevel = 'INFO';
            } else if (parsedUrl.path.endsWith('debug')) {
              logLevel = 'DEBUG';
            }
            return applicationLogUtils.setDynamicLogLevel(req, logLevel).then(()=>{
              res.end();
            });
          } else if (req.method === 'DELETE') {
            return applicationLogUtils.unsetDynamicLogLevel(req).then(()=>{
              res.end();
            });
          }
